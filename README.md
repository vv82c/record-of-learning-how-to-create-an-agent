...
- **双入口**：桌面窗口与终端 REPL 共用同一内核（`run_app.py` / `main.py`；终端 `/mi` 进密折）
...
- **Hooks 生命周期**：Event → Matcher → Handler → Decision 四层模型，内置安全策略（敏感文件管控/危险命令拦截/SSRF 防护）、审计日志、输出截断、Stop 质量门禁；统一守卫入口三端收编——主循环/子代理/队友同过一条链，无确认者语境 ask 自动拒绝（fail-closed）；网络出访命令（curl/wget/urllib 等）走圣旨确认；SSRF 防护自适应本机环境——hosts 式加速器域（用户配置的回环映射）自动生成白名单放行（限 80/443），无加速器机器零变化
- **稳定性**：轮级异常安全网——模型吐坏 JSON 参数就地补配对 tool 消息（协议保对，会话永不悬空 400），单轮崩溃降级为报错后继续会话（进程不死、会话不废）；批量工具一个被拦不再整轮终止（如实汇报其余结果），子代理模型调用带指数退避重试、失败降级为文字回禀；子代理出门前注入本机网络路况说明，超轮/熔断收兵自动携带途中已到手的线索回禀总管
- **流式输出**：回答逐 token 上屏，工具调用碎片按 index 拼装
- **多会话管理**：`/new` 开新会话、`/resume` 跨进程恢复、`/find` 按题名/内容检索、`/export` 导出 Markdown 话本；会话全保真存储，界面端支持删除/改名/搜索
- **密折（临时对话）**：不入名册、不留记忆、关窗即焚；`save_memory` 等留痕工具自动禁用，`/export`（终端 `/mi` 进入）可手动誊出逃生；工具照常受圣旨管控
