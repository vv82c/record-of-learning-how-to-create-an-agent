...
- **Hooks 生命周期**：Event → Matcher → Handler → Decision 四层模型，内置安全策略（敏感文件管控/危险命令拦截/SSRF 防护）、审计日志、输出截断、Stop 质量门禁；统一守卫入口三端收编——主循环/子代理/队友同过一条链，无确认者语境 ask 自动拒绝（fail-closed）
