...
- **Hooks 生命周期**：Event → Matcher → Handler → Decision 四层模型，内置安全策略（敏感文件管控/危险命令拦截/SSRF 防护）、审计日志、输出截断、Stop 质量门禁；统一守卫入口三端收编——主循环/子代理/队友同过一条链，无确认者语境 ask 自动拒绝（fail-closed）；网络出访命令（curl/wget/urllib 等）走圣旨确认
- **稳定性**：轮级异常安全网——模型吐坏 JSON 参数就地补配对 tool 消息（协议保对，会话永不悬空 400），单轮崩溃降级为报错后继续会话（进程不死、会话不废）；批量工具一个被拦不再整轮终止（如实汇报其余结果），子代理模型调用带指数退避重试、失败降级为文字回禀
