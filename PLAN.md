...
- Backlog 现存 5 条改进候选（债②沙箱根治、圣旨规则持久化、子代理只读预设、微压缩、schema 用语中性化，见文末）
...
## 阶段十四：债③+债⑤ 合并清偿——如实汇报与出巡容错（P13 — 2026-09-20 立项）

> 债③（批量拦截早退）：本批工具只要有一个被 Hook 拦，整轮终止、同批成功结果陪葬——
> 固定文案替模型说话，违背"如实报告工具结果"的行事规矩第 5 条（4.1 端到端实测留痕）。
> 债⑤（子代理 LLM 裸奔）：小太监每轮模型调用无重试无兜底——限流一次、同批三个小太监的
> 成果全丢（阶段十一安全网已兜住"进程不死"，本笔还的是体验账）。

- [x] **14.1 债③ 批量拦截继续循环**（✅ 2026-09-20，去掉 `_run_loop` 尾部 early-return：工具消息已全部入史，注入防重试 user 提醒（照抄 Stop 门禁手法）后继续循环；`_last_real_user_index` 增 SYNTHETIC_USER_PREFIXES 常量统一跳过 Stop 门禁与本提醒两类合成消息）
  - 【完成标志】断言 A1~A6/B1~B3：同批 current_time 成功 + read .env 被拒 → 第二轮 LLM 照常发生、最终回复是模型文本、提醒入史、成功与拒绝都在史、固定文案退场、done 照发；另拟回滚跳过提醒锚定原始圣谕
- [x] **14.2 债⑤ 子代理 LLM 重试兜底**（✅ 2026-09-20，RETRYABLE_ERRORS/MAX_LLM_RETRIES 迁 llm.py（runner/subagent 同一元组对象，断言 identity 取证）；`_call_llm_with_retry` 指数退避 1s→2s→仍败降级；`_llm_failure_message` 文字回禀；出巡簿 outcome="llm_error"）
  - 【完成标志】断言 C1~C2/D1~D2/E1：限流两次第三次成功（退避 1s→2s 取证）、连败降级回禀含"已重试 3 次"、llm_error 日志入簿、client=None 给引导文案
- [x] **14.3 验证与收尾**（✅ 2026-09-20，内核断言 **14 项全绿**（fake call_llm/client，不烧真 token）；终端/REST 冒烟；浏览器真跑复现 4.1 场景——同批 current_time ✓ + read .env ✗，模型诚实汇报两件结果且复述"不敢换名重试"，自动题名「汇报工具执行结果」；五文档同维护，UIPLAN 本阶段无 UI 涉及不更新）

## 阶段十五：出访路况自适应——hosts 白名单与探子知情权（P14 — 2026-09-22 立项）

> 背景：2026-09-21 实战查访事故（真跑取证）：本机 hosts 式 GitHub 加速器把 github.com 等
> 31 个域映射到 127.0.0.1，SSRF 防护（任务 2.3）按"解析到回环"一律拦截——加速器越尽责，
> 查访越瘫痪。东厂探事 web_fetch 连拦两轮、curl 被 fail-closed 拒，烧满 15 轮收兵，且第 5 轮
> 已读到的答案没能带回（超轮回禀只有统计没有线索），总管只得亲自重查一遍。
> 方案口径（与皇上议定）：①白名单自动从 hosts 生成，零手工维护，无加速器机器白名单自然
> 为空、行为零变化——不做全局 yolo；②探子出门前知路况、收兵时带线索；会话级出访许可暂缓。

- [x] **15.1 hosts 回环域名白名单**（✅ 2026-09-22，tools.py 新增 `hosts_loopback_domains()`：读 hosts（SystemRoot 与 /etc/hosts 候选路径，mtime 缓存，加速器改文件自动重读）收集映射到回环地址的域名；`assert_url_allowed` 加短路——白名单域仅放行 80/443 端口，堵"借 github.com:6379 摸本地其他服务"的口子；localhost 永不放行、IP 字面量照拦、重定向复检自动继承白名单；映射到非回环内网段（公司 10.x 等）的条目不背书。已知边界如实留痕：仅覆盖 hosts 式加速器，TUN/fake-ip 型（198.18.0.0/15）不适用）
  - 【完成标志】断言 A 组 7 项：加速器域 443 放行／6379 端口仍拦／回环 IP 字面量仍拦／localhost 仍拦且不入白名单／非回环映射不收照常拦／无 hosts 文件白名单为空不炸／注释行不误收
- [x] **15.2 探子路况知情**（✅ 2026-09-22，subagent.py 新增 `_network_env_note`：仅当身份工具含 web_fetch/run_command 且本机确有加速器映射时生成说明（无加速器 → 空串 → 全链路零变化）；run_subagent 把说明拼进子代理实际收到的差事头部；出巡簿 start 事件 task 保持差事原文、路况另记 env_note 字段，不挤占截断口径）
  - 【完成标志】断言 B 组 3 项：探子差事头部含说明与域名且原差事附后／无加速器零注入／无网络工具身份零注入；日志 task 原文与 env_note 独立取证
- [x] **15.3 收兵线索打捞**（✅ 2026-09-22，新增 `_salvage_clues`：从消息史捞最近 3 条非失败的工具结果（各压平截 150 字）；熔断／超轮／LLM 降级三条收兵路径统一追加"途中线索"段——超轮 ≠ 颗粒无收，答案不再丢在路上，总管免于重查）
  - 【完成标志】断言 C 组 4 项：超轮回禀带线索含具体内容／纯失败熔断无线索噪音／一成一败熔断带出成功线索／llm_error 降级不炸
- [x] **15.4 验证与收尾**（✅ 2026-09-22，内核断言 **25 项全绿**（fake call_llm/execute_guarded，不烧真 token）；终端冒烟真跑"找 zcode 仓库网址"**31s** 办结（事故当天同题 125s），web_fetch 直连 github 成功不再绕必应拼条目；真 LLM 探子直连取证：直取仓库描述并经 api.github.com 交叉核对一致（事故当天两域皆被拦）、curl 照旧 fail-closed 拒（阶段十/十二安全墙未松动）、路况说明入出巡簿；测试日志即跑即清。五文档同维护，UIPLAN 本阶段无 UI 涉及不更新）
...
| 2026-09-22 | 新增阶段十五并完成（15.1~15.4，2026-09-21 查访事故立项）：SSRF hosts 回环白名单（自动从 hosts 生成、限 80/443、非回环映射不背书）；探子差事注入路况说明（出巡簿 env_note 独立字段）；熔断/超轮/LLM 降级三条收兵路径统一带途中线索。内核断言 25 项全绿 + 终端冒烟 31s（事故当天同题 125s）+ 真 LLM 探子直连成功且 curl 照旧 fail-closed 拒 | 实战暴露"加速器被自家 SSRF 防护误拦、探子烧满轮次且把已到手的答案丢在路上"；方案与皇上议定：白名单自动生成零维护、不做全局 yolo、会话级出访许可暂缓 |
